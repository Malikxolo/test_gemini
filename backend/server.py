...
ACTIVATION_TIMEOUT_SECONDS = (
    5  # Short window - just enough for the response to complete
)
...
class ActivationManager:
    ...
    def __init__(self):
        ...
        self.is_conversation_stale = (
            lambda: (time.time() - self.last_bot_response_time) > 5
            if self.last_bot_response_time
            else True
        )
...
async def execute_tool(fc, tool_name: str, args: dict):
    ...
    try:
        ...
        if tool_name == "activate_response":
            ...
            is_followup = (
                state.activation.conversation_active
                and not state.activation.is_conversation_stale()
            )
        ...
        if state.gemini_session:
            try:
                ...
                if (
                    ...
                    logger.debug(
                        f"Session disconnected during {tool_name} response (reconnecting...)"
                    )
                else:
                        ...
                        async for response in turn:
                            ...
                            if response.tool_call:
                                for fc in response.tool_call.function_calls:
                                    ...
                                    if tool_name == "activate_response":
                                        state.activation.is_activated = True
                                        logger.info(
                                            f"🎙️ Bridge OPENED: {args.get('reason', '')}"
                                        )
                            ...
                            if (
                                ...
                                if (
                                    ...
                                    and state.activation.last_bot_response_time
                                ):
                                    current_time = time.time()
                                    ...
                                    if (
                                        time_since_last_response
                                        < LAST_RESPONSE_FOLLOWUP_WINDOW
                                    ):
                                        if not state.activation.is_activated:
                                            state.activation.is_activated = True
                                            logger.info(
                                                f"🎙️ Auto-activated for follow-up (within {time_since_last_response:.1f}s of last response)"
                                            )
