...
from backend.config import (
    ...
    RAG_API_URL,
    BOT_API_KEY,
    RECALLAI_API_KEY as RECALL_API_KEY,
    WEBHOOK_BASE_URL as PUBLIC_URL,
)
...
# WAKE WORDS - Bot will activate when it hears any of these
WAKE_WORDS = [
    # English
    "assistant",
    "hey assistant",
    ...
    "mochan ji",
    "assistant ji",
    # Common variations (STT errors)
    "asistant",
    "assistnt",
    "assistent",
]

# STOP WORDS - Bot will deactivate when it hears any of these
STOP_WORDS = [
    # English
    "thank you",
    "thanks",
...
# Activation timing
ACTIVATION_TIMEOUT_SECONDS = 45  # Extended for conversation
DEACTIVATE_AFTER_TURN = False  # Stay active until stop word or timeout
...
# Behavior settings
REQUIRE_ACTIVATION_FOR_AUDIO = True  # Must hear wake word first
...
def check_wake_words(text: str) -> bool:
...
class ActivationManager:
    ...
    def __init__(self):
        ...
        self.last_bot_response_time: float = (
            None  # Track when bot last responded for follow-up detection
        )
    ...
    async def on_turn_complete(self):
        ...
        async with self._lock:
            if DEACTIVATE_AFTER_TURN:
                if self.is_activated and LOG_ACTIVATION_EVENTS:
                    logger.info(
                        f"😴 Turn complete - DEACTIVATING (fresh activation required)"
                    )
                self.is_activated = False
                self.current_turn_activated = False
            else:
        ...
        if tool_name == "activate_response":
            ...
            if state.gemini_session:
                function_response = types.FunctionResponse(
                    ...
                    response={
                        "status": "activated",
                        "message": "Voice channel is now open.",
                    },
                )
                await state.gemini_session.send(
...
DEFAULT TO NOT CALLING THIS FUNCTION unless you're confident they explicitly addressed you.""",
    ...
    system_instruction = f"""You are {BOT_NAME}, a helpful AI assistant in a Google Meet call.
...
Be helpful, concise, and responsive. Match the speaker's language (Hindi or English).
                        ...
                        async for response in turn:
                            ...
                            if response.tool_call:
                                for fc in response.tool_call.function_calls:
                                    ...
                                    if tool_name != "activate_response":
                                        ...
                                        query_key = (
                                            f"{tool_name}:{query.lower().strip()}"
                                        )
                                        current_time = time.time()
                                        if query_key in state.pending_queries:
                                            if (
                                                current_time
                                                - state.pending_queries[query_key]
                                                < state.DEDUPE_WINDOW
                                            ):
                                                continue
                                    ...
                                    if tool_name == "activate_response":
                                        ...
                                        logger.info(
                                            f"🎙️ Bridge OPENED: {args.get('reason', '')} | Topic: {topic}"
                                        )

                                    asyncio.create_task(
                                        execute_tool(fc, tool_name, args)
                                    )

                            # Log transcriptions and check wake/stop words
                            if (
                                ...
                                # Check wake words to activate
                                if not state.activation.is_activated:
                                    if check_wake_words(transcript_text):
                                        state.activation.is_activated = True
                                        state.activation.activation_time = time.time()
                                        state.activation.total_activations += 1
                                        logger.info(
                                            f"🎙️ WAKE WORD detected! Bot ACTIVATED"
                                        )

                                # Check stop words to deactivate
                                elif state.activation.is_activated:
                                    if check_stop_words(transcript_text):
                                        state.activation.is_activated = False
                                        logger.info(
                                            f"😴 STOP WORD detected! Bot DEACTIVATED"
                                        )
                                ...
                                for part in response.server_content.model_turn.parts:
                                    if part.inline_data and isinstance(
                                        ...
                                        if state.activation.is_activated:
                                            ...
                                            if audio_chunk_count == 1:
                                                logger.info(
                                                    "🔇 Audio suppressed (not activated)"
                                                )
                            ...
                            if (
                                ...
                                if audio_chunk_count > 0:
                                    ...
                                    state.activation.last_bot_response_time = (
                                        time.time()
                                    )
                                audio_chunk_count = 0
